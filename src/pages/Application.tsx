...
function Application() {
  ...
  const [app, setApp] = useState(appItem.sort((el1, el2)=> el1.title > el2.title? 1 : -1))
  useLayoutEffect(() => {
    ref.current?.focus();
  });
  ...
  function handleSearch(e : ChangeEvent<HTMLInputElement>){
	...
	if(value === ''){
		...
		matchApp = appItem.filter((app) => app.title.toLowerCase().startsWith(value.toLowerCase()))
		setApp(matchApp)
	}
  }
  return (
    <div className="min-w-96 px-8 min-h-96 flex justify-start flex-col items-center bg-primaryColor shadow-2xl rounded-md w-full max-w-5xl h-[600px]" ref={refDiv}>
